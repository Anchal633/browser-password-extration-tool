...
def extract_passwords():
    ...
    try:
        ...
        total_estimate = sum([len([f for f in os.listdir(paths['PATH']) if re.search(r"^Profile*|^Default$", f)]) for _, paths in BROWSERS.items()])
        done = 0
        ...
        with open(csv_filename, mode='w', newline='', encoding='utf-8') as file:
            ...
            for browser, paths in BROWSERS.items():
                ...
                log_area.insert(tk.END, f"Extraction complete for {browser}: {count} passwords\n")
...
root = tk.Tk()
root.title("Edge Password Decryption Tool")
...
tk.Label(root, text="Edge Password Decryption Tool", font=("Arial",16), bg="#1e1e1e", fg="white").pack(pady=10)
...
frame_sort = tk.Frame(root, bg="#575757")
frame_sort.pack()
